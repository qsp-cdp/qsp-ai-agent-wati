// ============================================================================================
// RETIRADA el 21-ago-2026 · RESPALDO DEL CÓDIGO ORIGINAL (versión 63, la última que estuvo viva)
// ============================================================================================
// Esta era la captura de direcciones del sistema ANTERIOR, invocada por un chatbot de WATI. NO estaba
// versionada en git (se subió a mano), así que se rescató de producción con get_edge_function antes de
// retirarla. Este archivo vive FUERA de supabase/functions/ a propósito: el workflow de CI despliega
// todo directorio que encuentre ahí, y esto es un archivo muerto, no una función.
//
// POR QUÉ SE RETIRÓ
//   1. El negocio confirmó que su chatbot ya no se usa, y los registros lo respaldan: 0 llamadas en 24h.
//   2. Escribía los MISMOS datos que el copiloto con OTROS nombres de atributo (`maps_envio` vs
//      `pin_envio`), y la ficha del cliente terminaba mostrando dos direcciones y dos pines distintos
//      a la vez. El copiloto adoptó el esquema completo en v89 y quedó como único escritor en vivo.
//   3. RIESGO ACTIVO: con `es_correccion` en verdadero ponía a null la referencia, el pin y las
//      coordenadas de la libreta — borrando lo que el copiloto acababa de capturar.
//
// QUÉ HACE HOY ESE TRABAJO
//   · Captura conversacional: copilot-webhook (tool guardar_datos_envio).
//   · Captura pedida por el asesor: copilot-webhook?captura=1 (chatbot "Captura con AI").
//   · Ver docs/captura-envio.md.
//
// PARA RESTAURARLA (si hiciera falta): copiar este archivo a supabase/functions/wati-address/index.ts
// y empujarlo — el CI lo desplegaría. Requiere el secreto WATI_WEBHOOK_TOKEN, que sigue configurado.
// ============================================================================================
