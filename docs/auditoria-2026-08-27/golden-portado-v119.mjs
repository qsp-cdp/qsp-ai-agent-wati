...
function extraerFuncion(nombre) {
  ...
  // [AUDIT] strip de anotaciones de tipo en declaraciones const/let del cuerpo (v119 las usa)
  body = body.replace(/\b(const|let)\s+([A-Za-z_$][\w$]*)\s*:\s*[A-Za-z0-9_$\[\]<>|., ]+=/g, "$1 $2 =");
  // [AUDIT] casts TS `x as Tipo` dentro del cuerpo
  body = body.replace(/\s+as\s+[A-Za-z_$][\w$.<>\[\]|]*/g, "");
  const _f = eval(`(${sig}${body})`);
  // [AUDIT] blindaje: si la función extraída truena por dependencias fuera de scope, cuenta como fallo sin abortar
  return (...a) => { try { return _f(...a); } catch { return undefined; } };
}
...
const migAck = readFileSync(join(dirname(fileURLToPath(import.meta.url)), "..", "supabase", "migrations", "20260825170000_acks_y_cierres_de_conversacion.sql"), "utf8");
